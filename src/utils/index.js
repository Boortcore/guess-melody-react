export function getStatistic(results, hashId) {
  ...
  const betterPlayers = results.filter((result) => result !== currentResult && (result.correctAnswers > currentResult.correctAnswers
    || result.correctAnswers >= currentResult.correctAnswers && result.time < currentResult.time)).length;
